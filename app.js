(() => {
  ...
  const SUPABASE_ANON_KEY = 'sb_publishable_FzDjgkUIrncKMuyt253GsA_qK2fGKH1';
  ...
  const createProjectTitleInput = document.getElementById('create-project-title');
  ...
  async function insertProject(title, projectData) {
    const { data, error } = await supabaseClient
      .from(PROJECTS_TABLE)
      .insert({ title, data: projectData })
      .select('id, title, data')
      .single();
    if (error) throw error;
    return data;
  ...
  async function updateProjectData(id, projectData) {
  ...
  // Normalizes the jsonb "data" column from a Supabase project row (already
  // a parsed object, not a JSON string) into the same {tags, tasks} shape.
  function normalizeProjectData(data) {
    const rawTasks = data && Array.isArray(data.tasks) ? data.tasks : [];
    const rawTags = data && Array.isArray(data.tags) ? data.tags : DEFAULT_TAG_OPTIONS;
    return buildTagsAndTasks(rawTags, rawTasks);
  }
  ...
  function applyProjectRow(row) {
    const parsed = normalizeProjectData(row.data);
    currentProjectId = row.id;
    currentProjectTitle = row.title || 'Untitled';
    tasks = parsed.tasks;
  ...
  async function createProject(title) {
    ...
    try {
      const row = await insertProject(title, buildSeedProjectData());
      applyProjectRow(row);
    } catch (err) {
      console.error(err);
      setFileStatus('Could not create project: ' + err.message, 'error');
    }
  }
  ...
  function disconnectProject(emptyMessage) {
    ...
    setPageTitleEditable(false);
    setFileStatus('No project connected', '');
  }

  // Removing hides the project from the Open Project list by flagging it in
  // the database (the "archived" column) rather than deleting its data —
  // an administrator could still restore it directly in Supabase.
  async function removeCurrentProject() {
    if (!currentProjectId) return;
    const title = currentProjectTitle || 'this project';
    if (!window.confirm(`Remove "${title}" from the project list? This hides it — the data isn't deleted.`)) return;
    try {
      await archiveProject(currentProjectId);
      disconnectProject('Ready for the next project!');
    } catch (err) {
      console.error(err);
      setFileStatus('Could not remove project: ' + err.message, 'error');
    }
  }
  ...
  async function openProjectsDialog() {
    ...
    try {
      const rows = await fetchProjectList();
      renderProjectsList(rows);
    } catch (err) {
      console.error(err);
      projectsListEl.innerHTML = '';
  ...
  function renderProjectsList(rows) {
    ...
    if (!rows.length) {
      const li = document.createElement('li');
      li.className = 'filter-empty';
      li.textContent = 'No projects yet — use "Create Project" to start one.';
      projectsListEl.appendChild(li);
      return;
    }
    rows.forEach((row) => {
      ...
      titleSpan.textContent = row.title || 'Untitled';
      btn.appendChild(titleSpan);

      const updated = row.updated_at ? new Date(row.updated_at) : null;
      if (updated && !Number.isNaN(updated.getTime())) {
        const metaSpan = document.createElement('span');
        metaSpan.className = 'project-item-meta';
        metaSpan.textContent = 'Updated ' + formatShort(updated);
        btn.appendChild(metaSpan);
      }

      btn.addEventListener('click', async () => {
        projectsDialog.close();
        await loadProjectById(row.id);
      });
      li.appendChild(btn);
      projectsListEl.appendChild(li);
    });
  ...
  async function tryRestoreLastProject() {
    let lastId = null;
    try {
      lastId = localStorage.getItem(LAST_PROJECT_STORAGE_KEY);
    } catch (err) {
    ...
    const ok = await loadProjectById(lastId);
    if (!ok) {
  ...
  function setPageTitleEditable(editable) {
    pageTitleEl.disabled = !editable;
    pageTitleEl.title = editable ? 'Click to rename this project' : '';
  }

  async function renameCurrentProject(rawTitle) {
    if (!currentProjectId) return;
    const title = rawTitle.trim();
    if (!title || title === currentProjectTitle) {
      pageTitleEl.value = currentProjectTitle;
      return;
    }
    const previous = currentProjectTitle;
    currentProjectTitle = title;
    applyPageTitle(title);
    setFileStatus('Connected: ' + currentProjectTitle, 'connected');
    try {
      await updateProjectTitle(currentProjectId, title);
    } catch (err) {
      console.error(err);
      currentProjectTitle = previous;
      applyPageTitle(previous);
      setFileStatus('Could not rename project: ' + err.message, 'error');
    }
  }
  ...
  editDeleteBtn.addEventListener('click', () => {
    ...
    if (id == null) return;
    const name = editNameInput.value.trim() || 'this task';
  ...
  async function saveTasks() {
    if (!currentProjectId) return;
    try {
      await updateProjectData(currentProjectId, projectDataToPayload(tasks, tagOptions));
  ...
  openFileBtn.addEventListener('click', openProjectsDialog);
  newFileBtn.addEventListener('click', openCreateProjectDialog);
  projectsCloseBtn.addEventListener('click', () => projectsDialog.close());
  createProjectCancelBtn.addEventListener('click', () => createProjectDialog.close());
  createProjectForm.addEventListener('submit', async (e) => {
    e.preventDefault();
    const title = createProjectTitleInput.value.trim();
    if (!title) return;
  ...
  removeProjectBtn.addEventListener('click', removeCurrentProject);

  pageTitleEl.addEventListener('change', () => renameCurrentProject(pageTitleEl.value));
